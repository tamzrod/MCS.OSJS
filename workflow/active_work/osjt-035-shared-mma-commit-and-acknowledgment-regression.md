# OSJT-035 — Shared MMA commit and acknowledgment regression
...
## Primary outcome
Use existing transaction helpers to commit a revision-checked candidate and wait for acknowledgment; no-op does not restart, committed failure returns recovery-required without unsafe rollback. (independent deterministic verification only).
...
From simulator: go test -count=1 -timeout=90s -v -run '^TestSharedSettingsCommit' . Capture full output showing matching tests actually ran and passed; zero matching tests is not PASS. No source edits.
...
Depends on OSJT-034; target/permissions derive from OSJT-001.

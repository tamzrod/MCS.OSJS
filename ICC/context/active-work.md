...
7. `sim-005-osjs-window.md` — OS.js Modbus Simulator window.**CURRENT — execute first.** Backend persistence layer committed at HEAD `b69c46c` (JSON tags on simulator model `simulator/device.go`; `Store.SaveDocument(Document)` multi-device atomic save `simulator/store.go` (SaveOne delegates(; `simulator/store_document_test.go` 3 tests; `simulator/bridge.go`+`simulator/bridge_test.go` loopback JSON bridge `BridgePath=/api/devices` GET/PUT on simulator-owned document only; `gofmt -l .` clean;`go vet ./...` clean;`go test -count=1 ./...` ok(. Checkpoint pushed `575a413`: OS.js server proxy route added (`OSJS/src/server/providers/simulator-bridge.js` registered in `OSJS/src/server/index.js`; same-origin `GET/PUT /api/devices` → `127.0.0.1:18211`; 503 when bridge down; body through Express `bodyParser.json`; **Modbus Simulator window package WIP** (`OSJS/src/packages/ModbusSimulator/`: metadata.json, package.json, webpack.config.js, index.js, index.scss placeholder(; `index.js` header/imports valid but implementation body carries KNOWN CORRUPTION (mixed-width/Arabic-digit/typo artifacts from mid-edit accidents; syntax-invalid; needs complete ASCII rewrite(; `index.scss` content + build (`npm run build:local-packages`) + browser smoke test remain pending.**

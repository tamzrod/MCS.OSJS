# OSJT-027 — Advanced settings round trip
...
## Primary outcome
Demonstrate advanced settings persist in both OS.js editors.
...
For each editor on synthetic data, save an access policy and sealing configuration, reload and compare exact fields, then edit/discard. Save an RBE rule without enabling output; confirm globally unique IDs on duplicate. Restore the synthetic baseline. Use only the target and permissions recorded by OSJT-001. Record actual UI actions, replies, before/after values and cleanup evidence. Do not use production, public outputs, or the revoked runner.
...
Depends on OSJT-026; target/permissions derive from OSJT-001.

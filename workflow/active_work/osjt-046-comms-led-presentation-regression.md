# OSJT-046 — COMMS LED presentation regression
...
## Primary outcome
Wire evidence into four LEDs with timestamps/errors in tooltips, pulse only on new success, clear stale evidence and honor reduced motion. (independent deterministic verification only).

## Bounded scope
OSJS/tests/toolkit-comms-leds.test.js
...
From OSJS: node --test tests/toolkit-comms-leds.test.js. The file must contain nonzero assertions/cases for the predecessor behavior. Then record full output and exit 0; no source edits.
...
Depends on OSJT-045; target/permissions derive from OSJT-001.

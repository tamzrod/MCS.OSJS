...
UMIG CODE tasks: ChatGPT coding agent owns product changes, source readback, source checkpoint, test packets and workflow advancement. UMIG TEST/VERIFY tasks: OpenHands acts as JR and follows `operation cwal.md`; OpenHands runs only the current test packet and returns raw evidence, never edits product files, modifies task state or fixes bugs. The coding agent reads actual OpenHands evidence before deciding PASS/completion. A source-only checkpoint is not proof of a test or runtime acceptance. FAIL/BLOCKED stops automatic advancement; fix only within an explicitly authorized coding task.

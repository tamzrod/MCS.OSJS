# OSJT-026 — Advanced editor persistence wiring regression
...
## Primary outcome
Mount the shared editor using Memory mma2 and Replicator mma2_advanced; save/discard/duplicate preserve fields and failures retain drafts. (independent deterministic verification only).

## Bounded scope
OSJS/tests/toolkit-advanced-persistence.test.js
...
From OSJS: node --test tests/toolkit-advanced-persistence.test.js. The file must contain nonzero assertions/cases for the predecessor behavior. Then record full output and exit 0; no source edits.
...
Depends on OSJT-025; target/permissions derive from OSJT-001.

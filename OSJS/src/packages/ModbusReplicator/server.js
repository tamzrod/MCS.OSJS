...
const ALLOWED = new Set(['load', 'apply', 'status', 'suggest']);

const runtimeSocket = () => path.join(process.env.OSJS_DATA_DIR || process.cwd(), 'run', 'modbus-replicator.sock');

const callRuntime = request => new Promise((resolve, reject) => {
  const body = Buffer.from(JSON.stringify(request));
  if (body.length > MAX_MESSAGE) return reject(new Error('Replicator request is too large'));
  ...
  let expected = null;
  let received = 0;
  const timeout = setTimeout(() => socket.destroy(new Error('Replicator runtime request timed out')), 25000);
  const finish = error => {
  ...
  socket.on('data', chunk => {
    ...
    if (expected !== null && (expected > MAX_MESSAGE || expected < 1)) return socket.destroy(new Error('Invalid Replicator runtime response'));
...
module.exports = () => ({
  onmessage(ws, respond, args) {
    ...
    if (!ws._osjs_client || !request || request.version !== VERSION || !ALLOWED.has(request.operation)) {
      respond({version: VERSION, request_id: request && request.request_id || '', ok: false, error: {code: 'INVALID_REQUEST', message: 'Rejected Replicator request'}});
      return;
    }
    callRuntime(request)

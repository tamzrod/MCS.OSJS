...
## Primary outcome
Connect read-only projection to canonical load under the existing writer lock without recursive locking or disk writes. (independent deterministic verification only).
...
From simulator: go test -count=1 -timeout=90s -v -run '^TestAdvancedProjectionLoad' . Capture full output showing matching tests actually ran and passed; zero matching tests is not PASS. No source edits.
...
Depends on OSJT-011; target/permissions derive from OSJT-001.

...
// ComposeDocumentDestinations is the independent public entry point. Manager
// Boot/Apply already own a broader lock and use composeDocumentDestinationsLocked.
func (s Store) ComposeDocumentDestinations(doc Document) (Document, mma2composer.EffectiveConfig, error) {
	var resolved Document
	var effective mma2composer.EffectiveConfig
	err := s.withWriterLock(func() error {
		var err error
		resolved, effective, err = s.composeDocumentDestinationsLocked(doc)
		return err
	})
	return resolved, effective, err
}

// composeDocumentDestinationsLocked requires the outer shared transaction lock.
func (s Store) composeDocumentDestinationsLocked(doc Document) (Document, mma2composer.EffectiveConfig, error) {
	resolved, err := s.ResolveDocumentDestinations(doc)
	if err != nil { return Document{}, mma2composer.EffectiveConfig{}, err }
	...
	for i := range resolved.Devices {
		...
		if err != nil { return Document{}, mma2composer.EffectiveConfig{}, fmt.Errorf("device %q: %w", device.Name, err) }
...
func unionArea(current *mma2composer.Area, start, count uint16) (*mma2composer.Area, error) {
	if current == nil { return &mma2composer.Area{Start: start, Count: count}, nil }
	lo := uint32(current.Start)
	hi := lo + uint32(current.Count)
	blockLo := uint32(start)
	blockHi := blockLo + uint32(count)
	if blockLo < lo { lo = blockLo }
	if blockHi > hi { hi = blockHi }
	span := hi - lo
	if span == 0 || span > 0xffff { return nil, fmt.Errorf("combined destination area span %d cannot be represented by MMA2", span) }
...
func destinationMemoryForBlocks(unitID uint16, blocks []PullBlock) (mma2composer.Memory, error) {
	if len(blocks) == 0 { return mma2composer.Memory{}, fmt.Errorf("at least one pull block is required") }
	memory := mma2composer.Memory{UnitID: unitID}
	for _, block := range blocks {
		var err error
		switch block.Function {
		case 1: memory.Coils, err = unionArea(memory.Coils, block.Start, block.Count)
		case 2: memory.DiscreteInputs, err = unionArea(memory.DiscreteInputs, block.Start, block.Count)
		case 3: memory.HoldingRegs, err = unionArea(memory.HoldingRegs, block.Start, block.Count)
		case 4: memory.InputRegs, err = unionArea(memory.InputRegs, block.Start, block.Count)
		default: return mma2composer.Memory{}, fmt.Errorf("unsupported pull block function %d", block.Function)
		}
		if err != nil { return mma2composer.Memory{}, err }
	}
...
func documentDestinationPorts(doc Document) []uint16 {
	...
	for _, device := range doc.Devices {
		if !device.Enabled || device.Destination.Port == 0 || seen[device.Destination.Port] { continue }

...
7. `workflow/active_work/sim-005-osjs-window.md` — SIM-005: build the approved OS.js Modbus Simulator window.**CURRENT — execute this first.** Checkpoint 2026-09-07: backend persistence + loopback JSON bridge committed (HEAD `b69c46c`(; loopback bridge runner added (`simulator/cmd/simbridge` → `127.0.0.1:18211`; PUT/GET `/api/devices` round-trip smoke-verified; simulator-owned document only, no MMA2 config artifacts(; OS.js server proxy route + Modbus Simulator window frontend remain pending.**

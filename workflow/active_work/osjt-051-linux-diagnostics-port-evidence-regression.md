# OSJT-051 — Linux diagnostics port evidence regression
...
## Primary outcome
Report configured listeners and locally observable ownership only; container/network-namespace limitations must be explicit; no privileged Docker socket or host access. (independent deterministic verification only).

## Bounded scope
OSJS/tests/toolkit-diagnostics-ports.test.js
...
From OSJS: node --test tests/toolkit-diagnostics-ports.test.js. The file must contain nonzero assertions/cases for the predecessor behavior. Then record full output and exit 0; no source edits.
...
Depends on OSJT-050; target/permissions derive from OSJT-001.

# OSJT-029 — Shared MMA revisioned read regression
...
## Primary outcome
Implement mma-load with whitelisted root settings and exact-config-byte revision under the existing shared lock; read only. (independent deterministic verification only).
...
From simulator: go test -count=1 -timeout=90s -v -run '^TestSharedSettingsLoad' . Capture full output showing matching tests actually ran and passed; zero matching tests is not PASS. No source edits.
...
Depends on OSJT-028; target/permissions derive from OSJT-001.

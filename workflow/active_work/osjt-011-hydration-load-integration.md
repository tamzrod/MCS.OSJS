...
## Primary outcome
Connect read-only projection to canonical load under the existing writer lock without recursive locking or disk writes.

## Bounded scope
simulator/store.go; simulator/runtime_server.go; simulator/advanced_projection_test.go; simulator/osjs_toolkit_settings_test.go
...
Depends on OSJT-010; target/permissions derive from OSJT-001.

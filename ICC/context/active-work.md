...
SIM-001…SIM-008 are complete+verified. SIM-010 through SIM-013 are complete( SIM-013 at `24fd5b1` guaranteed enabled-only transactional composition with authoritative validation(. SIM-014 — MMA2 RESTART-only Control — **COMPLETED 2026-09-08** at pushed commit `db2276c`:after a successful committed shared-MMA2-config change, ApplyStructural writes exactly one restart-request artifact beside the shared config, waits for every composed listener port to return ready,and clears the request only on confirmed readiness;;restart/readiness failure is surfaced truthfully without claiming apply success;;no other MMA2 lifecycle operation exists;;four restart tests+live MMA2 reload verified. SIM-015 — Run Simulation after MMA2 Apply — is now CURRENT. SIM-010…SIM-017 remain human-promotedin dependency order.

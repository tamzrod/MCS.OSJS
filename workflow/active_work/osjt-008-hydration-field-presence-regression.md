...
## Primary outcome
Represent omitted versus explicit null, false and empty advanced values without changing apply behavior. (independent deterministic verification only).
...
From simulator: go test -count=1 -timeout=90s -v -run '^TestAdvancedProjectionPresence' . Capture full output showing matching tests actually ran and passed; zero matching tests is not PASS. No source edits.
...
Depends on OSJT-007; target/permissions derive from OSJT-001.

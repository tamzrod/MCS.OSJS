# OSJT-014 — Advanced folder tabs regression
...
## Primary outcome
Add Device Definition and Advanced Settings folder tabs to both editors, preserving device drafts; no settings controls yet. (independent deterministic verification only).

## Bounded scope
OSJS/tests/toolkit-advanced-tabs.test.js
...
From OSJS: node --test tests/toolkit-advanced-tabs.test.js. The file must contain nonzero assertions/cases for the predecessor behavior. Then record full output and exit 0; no source edits.
...
Depends on OSJT-013; target/permissions derive from OSJT-001.

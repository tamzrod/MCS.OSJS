...
func (s Store) RunOnce() (CycleResult, error) {
	cfg, err := s.Load()
	if err != nil { return CycleResult{}, err }
	if err := validateCycleMapping(cfg); err != nil { return CycleResult{}, err }
	if err := s.composeDestination(cfg.Destination); err != nil { return CycleResult{}, err }
	// No source polling or raw ingest inside the shared configuration lock.
	...
	if err := sendDestination(cfg.Destination, payload); err != nil { return CycleResult{}, err }
	return CycleResult{
		Function: payload.Function, SourceStart: payload.Start,
		DestinationArea: strings.ToLower(strings.TrimSpace(cfg.Destination.Area)),
		DestinationStart: cfg.Destination.Start, Count: cfg.Destination.Count,
	}, nil
}

func validateCycleMapping(cfg Config) error {
	if cfg.Source.Count != cfg.Destination.Count { return fmt.Errorf("source.count and destination.count must match for 1:1 replication") }
	if cfg.Source.Function < 1 || cfg.Source.Function > 4 { return fmt.Errorf("replication supports FC1, FC2, FC3, and FC4 only") }
	area := strings.ToLower(strings.TrimSpace(cfg.Destination.Area))
	want := fmt.Sprintf("fc%d", cfg.Source.Function)
	if area != want { return fmt.Errorf("cross-area mapping is not supported: source fc%d to destination %s", cfg.Source.Function, area) }
	return nil
}

func sourcePayloadCount(payload RegisterValues) int {
	if payload.Function == 1 || payload.Function == 2 { return len(payload.Bits) }
	return len(payload.Values)
}

func (s Store) composeDestination(dst DestinationConfig) error {
	return s.withWriterLock(func() error { return s.composeDestinationLocked(dst) })
}

func (s Store) composeDestinationLocked(dst DestinationConfig) error {

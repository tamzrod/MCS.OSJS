...
Selected-branch state: `active-work` is refreshed over source checkpoint `7e3e4fd` plus the current transition overlay. OSJT-001 through OSJT-007 are archived; OSJT-008 is the sole ACTIVE TEST task and later tasks are QUEUED. `handoff.md` is the exact chat-only JR packet with clean-checkout, source-ancestry, changed-path, and remote-main freshness gates. `simulator-device-config` records the repaired fixture at `7e3e4fd`. Other semantic branches retain their own recorded baselines and were not refreshed.

...
`STAYING ALIVE` is the emergency sandbox-rescue command for MCS.OSJS.
...
`STAYING ALIVE` is explicitly authorized to:
...
If ICC itself needs maintenance, invoke `BLACK SHEEP WALL` only for the affected semantic branch. STAYING ALIVE must not edit `ICC/` directly.
...
`STAYING ALIVE` does not itself destroy an OpenHands/cloud sandbox unless the execution environment explicitly provides a safe sandbox-destruction capability.

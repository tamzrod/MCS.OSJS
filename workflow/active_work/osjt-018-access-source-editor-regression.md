# OSJT-018 — Access source editor regression
...
## Primary outcome
Add editable IP/CIDR input plus independent IPv4/IPv6 presets; normalize comma-separated entries without shifting draft rows. (independent deterministic verification only).

## Bounded scope
OSJS/tests/toolkit-access-sources.test.js
...
From OSJS: node --test tests/toolkit-access-sources.test.js. The file must contain nonzero assertions/cases for the predecessor behavior. Then record full output and exit 0; no source edits.
...
Depends on OSJT-017; target/permissions derive from OSJT-001.

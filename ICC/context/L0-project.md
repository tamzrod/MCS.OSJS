...
SIM-010 through SIM-012 are complete. SIM-013 completed at `24fd5b1` with enabled-only, foreign-preserving, authoritative validation and safe shared-config commit. SIM-014 (MMA2 RESTART-only control) is CURRENT, followed by SIM-015 through SIM-017. MMA2 auto-starts independently; Simulator may request only RESTART after a committed valid shared-config change. Detail in Zoom In `active-work`.

# OSJT-006 — Cross-editor ownership rejection
...
## Primary outcome
Demonstrate that Memory and Replicator cannot overwrite the same port/unit reservation.
...
Create a synthetic Memory reservation, then request its exact destination pair from Replicator with both Auto flags off; confirm rejection and unchanged documents. Check another Unit ID on that port is allowed. Restore only synthetic data. Use only the target and permissions recorded by OSJT-001. Record actual UI actions, replies, before/after values and cleanup evidence. Do not use production, public outputs, or the revoked runner.
...
Depends on OSJT-005; target/permissions derive from OSJT-001.

...
func destinationMemory(dst DestinationConfig) (mma2composer.Memory, error) {
	...
	case "fc1":
		mem.Coils = area
	case "fc2":
		mem.DiscreteInputs = area
	case "fc3":
		mem.HoldingRegs = area
	case "fc4":
		mem.InputRegs = area
	default:
		return mma2composer.Memory{}, fmt.Errorf("destination area must be fc1, fc2, fc3, or fc4")
	}
...
func sendDestination(dst DestinationConfig, payload RegisterValues) error {
	...
	if area == mma2raw.Coils || area == mma2raw.DiscreteInputs {
		values.Bits = payload.Bits
	} else {
		values.Registers = payload.Values
	}
	if err := client.Send(area, values); err != nil {
...
func rawArea(area string) (mma2raw.Area, error) {
	...
	default:
		return 0, fmt.Errorf("destination area must be fc1, fc2, fc3, or fc4")
	}
}

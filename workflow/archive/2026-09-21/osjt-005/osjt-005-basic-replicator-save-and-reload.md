# OSJT-005 — Basic Replicator save and reload
...
## Primary outcome
Demonstrate one working basic Replicator mapping through OS.js.
...
Using the approved local synthetic source, create one FC3 Pull Block and a distinct destination; Save & Apply, reload, and compare source/destination values. Preserve evidence and restore the recorded synthetic baseline. Use only the target and permissions recorded by OSJT-001. Record actual UI actions, replies, before/after values and cleanup evidence. Do not use production, public outputs, or the revoked runner.
...
Depends on OSJT-004; target/permissions derive from OSJT-001.
